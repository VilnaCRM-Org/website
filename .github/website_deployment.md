...
1. In-repository alerting and release audit

Logs are only useful if somebody is told to read them. Two workflows close that
gap (issue #383):
...
`make lint-prod-guardrails` fails a pull request if a privileged workflow is not
covered by that alerting, so the two cannot drift apart silently.
...
- A workflow's `name:` is load-bearing: `ci-health-alerts.yml` matches monitored
  workflows by name, so renaming one (for example `website`) requires updating
  that list in the same commit, or `make lint-prod-guardrails` fails the pull
  request.
