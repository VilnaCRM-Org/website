...
export const handleNetworkError:HandleNetworkErrorType =   ({networkError, setErrorDetails, setNotificationType, setIsNotificationOpen}:
...
export const handleApolloError:HandleApolloErrorType =
  ({ err, setErrorDetails, setNotificationType, setIsNotificationOpen}:HandleApolloErrorProps):void => {
  ...
  if (err.graphQLErrors?.length) {
    const message:string = err.graphQLErrors.map((e: GraphQLFormattedError) => e.message).join(', ');
