...
CI_LINT_TARGETS             = lint-next lint-tsc lint-md lint-api-versions lint-headers lint-prod-guardrails lint-node-version
...
format: ## This command executes Prettier formatting
	$(DEV_READY) $(PRETTIER_BIN) "**/*.{js,jsx,mjs,ts,tsx,json,css,scss,md}" --write --ignore-path .prettierignore
...
.PHONY: lint lint-api-versions lint-headers lint-docker-policy lint-security-txt lint-prod-guardrails lint-node-version
...
# Host-only in either EXEC_MODE, like lint-docker-policy: plain shell over files
# already in the checkout, so nothing comes from node_modules and requiring the
# dev container to be up would only slow the check down. It reads the repo's Node
# version sources and compares them with each other; the separate
# check-node-version target compares the *running* Node with package.json engines.
...
# no package manager, mirroring how lint-deps stays out. lint-node-version is
# host-only shell for the same reason, and joins CI_LINT_TARGETS because the
# parallel runner needs no package manager to run it.
lint: generate-localization lint-next lint-tsc lint-md lint-deps lint-api-versions lint-docker-policy lint-headers lint-security-txt lint-prod-guardrails lint-node-version ## Runs all linters: ESLint, TypeScript, Markdown, dependency-cruiser, the API version invariant, the Dockerfile registry/digest policy, the security-header gate, the RFC 9116 security.txt gate, the production-safety guardrails, and the Node version drift gate in sequence.
