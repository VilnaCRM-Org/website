...
@test "exempts a privileged workflow that only runs on pull requests" {
  ...
  # It still needs an `environment:` for assertion D -- being PR-scoped exempts a
  # workflow from the ALERTING requirement, never from the environment gate: a PR
  # branch is exactly the context #375 F1 shows can reach the production account.
...
# --- Assertion D: role-assuming jobs must be environment-gated -----------------
#
# Issue #375 F1/F2. `environment:` is the only key that lets a maintainer put a
# required reviewer in front of a production-account credential, and it is also
# what makes the minted OIDC subject `...:environment:<name>` rather than a bare
# branch ref that any branch in the repo can produce. It was absent from every
# workflow when #375 was filed; this assertion is what stops it being dropped
# again once a maintainer has created the environments.

@test "fails when a role-assuming job declares no environment" {
  cat >"$FIXTURE/.github/workflows/ungated-role.yml" <<'YAML'
name: ungated role
on:
  pull_request:
    branches:
...
YAML
  ...
  assert_output_contains 'ungated-role.yml'
  assert_output_contains '"provision"'
  assert_output_contains 'no `environment:` key'
}

@test "accepts a role-assuming job that declares an environment mapping" {
  cat >"$FIXTURE/.github/workflows/gated-role.yml" <<'YAML'
name: gated role
on:
  pull_request:
    branches:
...
@test "accepts the shorthand string form of environment" {
  # `environment: sandbox` is the documented shorthand for the mapping form and
  # attaches the identical protection rules, so it must not be reported.
  cat >"$FIXTURE/.github/workflows/gated-role-shorthand.yml" <<'YAML'
name: gated role shorthand
on:
  pull_request:
    branches:
      - main
...
@test "an empty environment name does not satisfy the gate" {
  # Boundary: `environment: ''` is present as a key but names no environment, so
  # no protection rule can attach and the OIDC subject gains no environment
  # segment. Testing for the key alone would accept it.
  cat >"$FIXTURE/.github/workflows/empty-env.yml" <<'YAML'
name: empty env
on:
  pull_request:
    branches:
...
YAML
  ...
  [ "$status" -eq 1 ]
  assert_output_contains '[D]'
  assert_output_contains 'empty-env.yml'
}

@test "an environment mapping carrying only a url does not satisfy the gate" {
  # Boundary: the mapping form without `name:` references no environment at all.
  cat >"$FIXTURE/.github/workflows/nameless-env.yml" <<'YAML'
name: nameless env
on:
  pull_request:
    branches:
...
YAML
  ...
  [ "$status" -eq 1 ]
  assert_output_contains '[D]'
  assert_output_contains 'nameless-env.yml'
}

@test "a job with no role-to-assume step is not asked for an environment" {
  # Negative case: the assertion stays scoped to jobs that actually mint a cloud
  # credential. Demanding an environment of every job would gate the whole test
  # matrix, and a gate that broad gets switched off rather than obeyed.
  cat >"$FIXTURE/.github/workflows/plain-job.yml" <<'YAML'
name: plain job
on:
  pull_request:
    branches:
...
@test "an environment on a sibling job does not cover the role-assuming one" {
  # The check is per JOB, not per file: an `environment:` on an unrelated job in
  # the same workflow protects nothing.
  cat >"$FIXTURE/.github/workflows/sibling-env.yml" <<'YAML'
name: sibling env
on:
  pull_request:
    branches:
...
jobs:
  gated:
    ...
    environment:
      name: sandbox
    steps:
      - run: echo nothing privileged here
  provision:
    runs-on: ubuntu-latest
    steps:
      - uses: aws-actions/configure-aws-credentials@v6
...
@test "fails when the sandbox creation trigger loses its environment key" {
  # Differential against the REAL workflow: this is the exact state the
  # repository was in when #375 was filed.
  python3 - "$FIXTURE/.github/workflows/sandbox-creating.yml" <<'PYTHON'
import sys

path = sys.argv[1]
source = open(path).read()
mutated = source.replace('    environment:\n      name: sandbox\n', '', 1)
assert mutated != source, 'fixture did not change; the environment block moved'
open(path, 'w').write(mutated)
PYTHON
  run_guardrails
  [ "$status" -eq 1 ]
  assert_output_contains '[D]'
  assert_output_contains 'sandbox-creating.yml'
}

@test "fails when the sandbox deletion trigger loses its environment key" {
  python3 - "$FIXTURE/.github/workflows/sandbox-deleting.yml" <<'PYTHON'
import sys
...
PYTHON
  ...
  [ "$status" -eq 1 ]
  assert_output_contains '[D]'
  assert_output_contains 'sandbox-deleting.yml'
}

@test "fails when the production deploy job loses its environment key" {
  python3 - "$FIXTURE/.github/workflows/deploy.yml" <<'PYTHON'
import sys
