...
# Function to run E2E tests in DIND mode using Makefile with working container setup
run_e2e_tests_dind() {
    local website_dir=$1
    echo "🎭 Running E2E tests in DIND mode (using Makefile with working container setup)"
...
# Function to run load tests in DIND mode using working container setup
run_load_tests_dind() {
    local website_dir=$1
...
main() {
    ...
    echo "🎉 All Playwright and load tests completed successfully!"
}
...
show_usage() {
    ...
    echo ""
    echo "This script runs E2E tests and load tests in Docker-in-Docker mode"
...
case "${1:-help}" in
    ...
    test-e2e)
        ...
        main "." && run_e2e_tests_dind "."
        ;;
    test-load)
        echo "📊 Running load tests only..."
        main "." && run_load_tests_dind "."
        ;;
    test-swagger)
        echo "📊 Running Swagger load tests only..."
        main "." && run_load_tests_swagger_dind "."
        ;;
    *)
        main "$@"
        ;;
