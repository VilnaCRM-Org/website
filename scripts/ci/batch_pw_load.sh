...
# Function to run E2E tests in DIND mode using Makefile
run_e2e_tests_dind() {
    local website_dir=$1
    echo "🎭 Running E2E tests in DIND mode using Makefile"
    run_make_with_prod_dind "test-e2e" "E2E tests" "$website_dir"
}

# Function to run visual tests in DIND mode using Makefile
run_visual_tests_dind() {
    local website_dir=$1
    echo "🎨 Running visual tests in DIND mode using Makefile"
    run_make_with_prod_dind "test-visual" "Visual tests" "$website_dir"
}

# Function to run load tests in DIND mode using Makefile
run_load_tests_dind() {
    local website_dir=$1
    echo "⚡ Running load tests in DIND mode using Makefile"
    run_make_with_prod_dind "load-tests" "Load tests" "$website_dir"
}
