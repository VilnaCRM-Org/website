...
run_batch_tests() {
    echo "🧪 Running Unit, Mutation, and Lint tests batch..."
    ...
    if run_unit_tests_dind > test-logs/unit.log 2>&1; then
    ...
    if run_mutation_tests_dind > test-logs/mutation.log 2>&1; then
        echo "✅ Mutation tests PASSED"
    else
        echo "❌ Mutation tests FAILED"
        failed_tests+=("mutation")
    fi
    
    echo "🔍 Running ESLint..."
    if run_eslint_dind > test-logs/eslint.log 2>&1; then
        echo "✅ ESLint PASSED"
    else
        echo "❌ ESLint FAILED"
        failed_tests+=("eslint")
    ...
    echo "🔍 Running TypeScript check..."
    if run_typescript_check_dind > test-logs/typescript.log 2>&1; then
        echo "✅ TypeScript check PASSED"
    else
        echo "❌ TypeScript check FAILED"
    ...
    echo "🔍 Running Markdown linting..."
    if run_markdown_lint_dind > test-logs/markdown.log 2>&1; then
        echo "✅ Markdown linting PASSED"
    else
        echo "❌ Markdown linting FAILED"
