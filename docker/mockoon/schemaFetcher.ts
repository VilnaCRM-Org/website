...
if (!SCHEMA_URL) {
  throw new Error(
    'Schema URL is not configured. Please set the MOCKOON_SCHEMA_URL environment variable.'
  );
}

const OUTPUT_DIR: string = path.join(__dirname);
const OUTPUT_FILE: string = path.join(OUTPUT_DIR, 'data.json');
const LOG_LEVEL: string = process.env.MOCKOON_LOG_LEVEL || 'info';
...
const MAX_RETRIES: number = Number(process.env.MOCKOON_MAX_RETRIES) || 3;
const TIMEOUT_MS: number = Number(process.env.MOCKOON_TIMEOUT_MS) || 5000;
  ...
  while (retries < MAX_RETRIES) {
    ...
    try {
      const response: Response = await fetch(SCHEMA_URL, {
        ...
        headers: {
          'User-Agent': 'Mockoon/SchemaFetcher',
