...
type AttemptOutcome = { settled: true } | { settled: false; error: Error };
...
async function waitBeforeRetry(attempt: number): Promise<void> {
  ...
  const backoffTime: number = backoffDelay(attempt);
  logger.info(`Retry attempt ${attempt}/${MAX_RETRIES} after ${backoffTime}ms`);
  await new Promise<void>(resolve => {
    setTimeout(() => resolve(), backoffTime);
  });
}

async function requestSchema(): Promise<Response> {
...
async function saveSchema(response: Response): Promise<void> {
...
// A digest mismatch is not a transport failure, so it does not go through the
// retry path — a retry re-downloads the same bytes.
//
// This discards the download and returns, which is not fail-open: the mock keeps
// serving the schema Apollo.Dockerfile seeded from `contracts/`, i.e. the
// reviewed contract rather than whatever the moved tag now points at. Exiting
// instead would crashloop the container (`restart: unless-stopped` in
// docker-compose.test.yml) and bury this message behind a healthcheck timeout.
// `make lint-contracts` is what turns the same drift into a red PR.
function discardUnverifiedDownload(error: SchemaIntegrityError): void {
  logger.error(error.message);
  logger.error(`Discarded the download; keeping the vendored schema at ${OUTPUT_FILE}`);
}

function logAttemptFailure(error: Error): void {
...
async function attemptFetch(attempt: number): Promise<AttemptOutcome> {
  await waitBeforeRetry(attempt);

  logger.info(
    `Fetching OpenAPI schema from: ${SCHEMA_URL}... (Attempt ${attempt + 1}/${MAX_RETRIES})`
  );

  try {
    await saveSchema(await requestSchema());
    return { settled: true };
  } catch (error) {
    if (error instanceof SchemaIntegrityError) {
      discardUnverifiedDownload(error);
      return { settled: true };
    }

    logAttemptFailure(error as Error);
    return { settled: false, error: error as Error };
  }
}

function handleExhaustedRetries(): void {
...
export async function fetchAndSaveSchema(): Promise<void> {
  ...
  for (let attempt: number = 0; attempt < MAX_RETRIES; attempt += 1) {
    const outcome: AttemptOutcome = await attemptFetch(attempt);
    if (outcome.settled) {
      return;
    }
